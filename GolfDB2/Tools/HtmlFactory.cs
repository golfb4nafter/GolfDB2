...
namespace GolfDB2.Tools
{
    public class HtmlFactory
    {
        ...
        //public static string MakeOuterTable(int eventId, int PlayListId, string connectionString)
        //{

        //    GolfDB2.Event evt = EventDetailTools.GetEventRecord(eventId, connectionString);
        //    GolfDB2.EventDetail eventDetail = EventDetailTools.GetEventDetailRecord(evt.id, connectionString);
        //    GolfDB2.HoleList holeList = ShotgunHoleCalculator.GetHoleListById(eventDetail.PlayListId, connectionString);

        //    string [] holes = holeList.HoleList1.Split(',');


        //    string begin = "<table><tr>\r\n";
        ...
        //        if (s == "1")
        //        {
        //            HtmlFactory.MakeNineTable(0, string names, string scoreCardId, string[] scores, string total)
        //        }
        ...
        //    return begin + "</tr></table>\r\n";
        //}

        public static string MakeTotalsTable(string division, string handicap, string gross, string net, string scoreCardId)
        {
            string table = "<table>\r\n";
            table += "<tr><th>Division</th><th>HC</th><th>Gross</th><th>Net</th></tr>\r\n";
            table += "<td><input style=\"width: 45px!important;\" type=\"text\" id=\"Division__SCID__\" name=\"Division__SCID__\" value=\"__DIV__\"/></td>\r\n";
            table += "<td><input style=\"width: 30px!important;\" type=\"text\" id=\"HC__SCID__\" name=\"HC__SCID__\" value=\"__HC__\"/></td>";
            table += "<td><input style=\"width: 30px!important;\" type=\"text\" id=\"Gross__SCID__\" name=\"Gross__SCID__\" value=\"__GROSS__\"/></td>\r\n";
            table += "<td><input style=\"width: 30px!important;\" type=\"text\" id=\"Net__SCID__\" name=\"Net__SCID__\" value=\"__NET__\"/></td>\r\n";
            table += "</table>\r\n";

            return table.Replace("__SCID__", scoreCardId).
                         Replace("__DIV__", division).
                         Replace("__HC__", handicap).
                         Replace("__GROSS__", gross).
                         Replace("__NET__", net);
        }

        public static string MakeNineTableRow(int i, int start, int scoreCardId, string[] scores)
        {
            return string.Format("<td><input style=\"width: 20px!important;\" type=\"text\" maxlength=\"2\" id=\"Hole{0}_{1}\" name=\"Hole{0}_{1}\" value=\"{2}\" /></td>",
                i,
                scoreCardId,
                scores[i - start]);
        }

        //public static string MakeNineTable(int start, bool bNames, GolfDB2.Event evt, GolfDB2.EventDetail eventDetail, GolfDB2.HoleList holeList, string connectionString)
        //{
        //    StringBuilder sbHead = new StringBuilder();

        //    if (!bNames)
        //        sbHead.Append("<table><tr><th>Player/Team</th>");
        //    else
        //        sbHead.Append("<table><tr>");

        //    for (int i = start; i < (start + 9); i++)
        //        sbHead.Append(string.Format("<th>{0}</th>", i + 1));

        //    sbHead.Append("<th>Total</th></tr>\r\n");

        //    StringBuilder sb = new StringBuilder();


        //    List<TeeTime> teeTimes = TeeTimeTools.GetTeeTimeTimesByEventId(evt.id, connectionString);
        ...
        //        GolfDB2.ScoreCard card = new ScoreCard() { EventId = evt.id, StartingHole = tt.HoleId, Division = , TeeTimeId =  tt.Id };
        ...
        //        if (!bNames)
        //            sb.Append(string.Format("<tr><td><input type=\"text\" id=\"Names{0}\" name=\"Names{0}\" value=\"{1}\" /></td>", scoreCardId, names));
        //        else
        //            sb.Append("<tr>");

        //        for (int i = start; i < (start + 9); i++)
        //        {
        //            sb.Append(MakeNineTableRow(i, start, scoreCardId, scores));
        //        }

        //        sb.Append(string.Format("<td><input style=\"width: 30px!important;\" type=\"text\" id=\"Total{0}\" name=\"Total{0}\" value=\"{1}\" /></td>", scoreCardId, total));
        //        sb.Append("</tr>\r\n");
        //    }
